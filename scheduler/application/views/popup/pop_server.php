...
<div class="col-5">
    <div align="right" class="form-group input-group">
        서버이름 : <input name="serach_word" id="serach_word" value="<?php echo $_GET['search_word']?>" class="form-control">
...
<div class="card mb-3">
    <div class="card-header">
        <i class="fa fa-table"></i> SERVER LIST
    </div>
    <div class="card-body">
            ...
            <div class="tablesaw-overflow">
                <table class="table table-bordered tablesaw" id="" width="100%" cellspacing="0">
                    ...
                    <?php foreach ($data as $val) { ?>
                        <tr>
                            <td><a href="#" onclick=insertop("<?php echo $val['server_info_id']?>","<?php echo $val['server_name']?>")><?php echo $val['server_info_id']?></a></td>
                            <td><a href="#" onclick=insertop("<?php echo $val['server_info_id']?>","<?php echo $val['server_name']?>")><?php echo $val['server_name']?></a></td>
                            <td><a href="#" onclick=insertop("<?php echo $val['server_info_id']?>","<?php echo $val['server_name']?>")><?php echo $val['server_ip']?></a></td>
                            <td><a href="#" onclick=insertop("<?php echo $val['server_info_id']?>","<?php echo $val['server_name']?>")><?php echo $val['server_comment']?></a></td>
                        </tr>
                    <?php } ?>
                    </tbody>
                </table>
...
<script type="text/javascript">
    function insertop(idx, name){
        try {
            opener.document.getElementById("server_info_id").value = idx;
            opener.document.getElementById("server_name").value = name;
        }
        catch(e) {
            alert(e.description);
        }
